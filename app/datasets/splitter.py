...
class DatasetSplitter:
    ...
    def _split_to_dict(
        ...
        return {
            "images": [asdict(image) for image in split.images],
            "annotations": [asdict(annotation) for annotation in split.annotations],
            "categories": [asdict(category) for category in split.categories],
        }
    ...
    def _save_summary(
        ...
        summary_path = output_directory / "split_summary.json"
    ...
    def export(
        ...
        if (
            ...
            raise RuntimeError("Split dataset first.")
